...
export function ResultsHeroSection({ hero }: { hero: HeroData }) {
  ...
  const conversionRate =
    leads > 0 ? Number(((clients / leads) * 100).toFixed(2)) : 0;

  const conversionData =
    leads > 0
      ? [{ name: "Lead → client", value: conversionRate }]
      : [];
          ...
          <div className="relative z-10">
            ...
            <div className="mt-7 grid gap-4 md:grid-cols-2">
              ...
              <HeroCard
                ...
                subvalue={
                  hero.snapshot ||
                  "Фактически полученный платеж за последний период."
                }
              />
            </div>
          </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-[22px] bg-white/[0.035] px-4 py-4">
          ...
          <div className="mt-3 text-sm leading-6 text-white/76">
            {[
              hero.stage,
              hero.physicalLocation,
              hero.teamSizeCore ? `${hero.teamSizeCore} core team` : null,
            ]
              .filter(Boolean)
              .join(" · ") || "Нет данных"}
          </div>
